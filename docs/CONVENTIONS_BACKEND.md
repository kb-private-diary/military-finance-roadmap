...
작업 전후로 `dev` 를 자주 최신화하는 게 핵심. Fork(GUI)든 CLI든 순서는 같다.

1. **dev 최신화**: 작업 시작 전 `dev` 를 pull 해서 최신 상태로 맞춘다
2. **개인 브랜치에서 작업**: 본인 브랜치(`dev_suyeon` 등)에서만 코드 작성
   - ⚠️ `dev` · `main` 에는 **절대 직접 커밋/푸시 ❌** (PR로만)
3. **커밋**: 커밋 컨벤션(§2)대로 작성
4. **개인 브랜치 push**: 원본 저장소의 **내 브랜치로** 올린다 (`origin/dev_suyeon`)
5. **PR 생성**: GitHub 에서 `개인 브랜치 → dev` PR (제목·본문 §3)
6. **리뷰 → 병합**: 리뷰 승인 후 병합. **Merge commit** 방식 (Squash·Rebase ❌: 개인별 커밋 이력 보존)
7. **배포 직전**: 모인 기능을 `dev → main` PR 로 병합

> 💡 충돌(conflict) 나면: 내 브랜치에서 `dev` 를 다시 pull(또는 merge) → 충돌 해결 후 다시 push
> 💡 병합된 개인 브랜치는 삭제해도 되고(GitHub), 계속 재사용해도 됨 (재사용 시 병합 후 `dev` pull 먼저)
