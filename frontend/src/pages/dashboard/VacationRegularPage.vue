...
const fetchDetail = async () => {
  if (!vacationId.value) {
    loadError.value = '정기 휴가 정보를 찾을 수 없습니다.';
  ...
  try {
    ...
    console.error(error);
    loadError.value = '정기 휴가 정보를 불러오지 못했습니다.';
  } finally {
    isLoading.value = false;
  }
};
...
const registerUsage = async () => {
  ...
  try {
    await dashboardApi.createVacation({ category: 'REGULAR', days });
    isAddFormOpen.value = false;
    newUsageDays.value = '';
    await fetchDetail();
  } catch (error) {
...
const deleteUsage = async (usage) => {
  ...
  isSubmitting.value = true;
  try {
    await dashboardApi.deleteVacation(usage.vacationId);
    await fetchDetail();
  } catch (error) {
    console.error(error);
    show(
...
<template>
  <div class="vacation-regular container py-4">
